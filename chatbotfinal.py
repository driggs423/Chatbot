...
def check_actions(current_intent, attributes, context):
    ...
    if (current_intent.action == "store Fetched"):
        df=pd.read_csv('Datafiles/Stores.csv',encoding='latin-1')
        print(df[(df['type']==attributes['type']) &  (df['Rating']>=int(attributes['rating'])) & (df['Location']==attributes['location']) & (df['Time']==attributes['time'])])
        #(df['Rating']==int(attributes['rating'])) & (df['Location']==attributes['location']) & (df['Time']==attributes['time'])]
    if (current_intent.action == "books displayed"):
        #import pandas as pd
        df=pd.read_csv('Datafiles/Books.csv',encoding='latin-1')
        print(df[(df['author']==attributes['author']) & (df['genre']==attributes['genre'])])
